...
const INITIAL_SETTINGS = { theme: 'default', darkMode: true, useSearch: false, apiKey: '', aiUnlocked: false, alarmSound: 'default', customAlarmSoundUrl: '', iCalUrl: '' };
const MUSIC_SOURCES = {
...
const Header = memo(({ onSettingsClick, onChatClick, hasApiKey }) => (
...
const MusicPlayerCard = memo(({ timer, onDelete, onUpdate }) => {
      ...
      <div className="bg-surface rounded-xl p-6 flex flex-col gap-4 animate-fade-in shadow-lg transition-all duration-300">
          <div className="flex justify-between items-center">
              <div>
                  ...
                  <p className="text-xs text-secondary font-semibold">🎵 Music Mode 🎵</p>
              </div>
...
const Timer = memo(({ timer, onDelete, onToggle, onReset, onUpdate, alarmSoundUrl, compact = false }) => {
...
const TimerList = memo(({ timers, onAdd, onDelete, onToggle, onReset, onUpdate, alarmSoundUrl }) => {
    return (
...
const AddTimerModal = memo(({ isOpen, onClose, onAddTimer }) => {
    const [label, setLabel] = useLocalStorage('addTimer-label', '');
    const [hours, setHours] = useLocalStorage('addTimer-hours', 0);
    const [minutes, setMinutes] = useLocalStorage('addTimer-minutes', 0);
    const [seconds, setSeconds] = useLocalStorage('addTimer-seconds', 0);
...
const SettingsModal = memo(({ isOpen, onClose, settings, onSettingsChange }) => {
...
const ChatBot = memo(({ isOpen, onClose, timers, onDeleteTimer, onToggleTimer, onResetTimer, alarmSoundUrl, onUpdateTimer }) => {
    ...
    useEffect(() => { try { if (process.env.API_KEY) ai.current = new GoogleGenAI({ apiKey: process.env.API_KEY }); } catch(e) { console.error("Failed to initialize GoogleGenAI", e); } }, []);
    ...
    const handleSend = async () => {
        ...
        try {
            // FIX: Pass the entire conversation history to the model to make the chatbot stateful.
            // This aligns with the expected usage for building a conversation with `generateContent`.
            const history = newMessages.map(msg => ({
                role: msg.role,
                parts: [{ text: msg.text }]
            }));
            const response: GenerateContentResponse = await ai.current.models.generateContent({ model: 'gemini-2.5-flash', contents: history });
            // FIX: The type of `response.text` can be inferred as `unknown`, causing a TypeScript error.
            // We cast it to `string` because the Gemini API documentation guarantees that the `text` property
            // on the `GenerateContentResponse` object will be a string for text-based responses.
            setMessages([...newMessages, { role: 'model', text: response.text as string }]);
        } catch (error) { console.error(error); setMessages([...newMessages, { role: 'model', text: 'Sorry, an error occurred.' }]); }
...
const NavButton = memo(({ tabName, icon, children, activeTab, onTabClick }) => (
...
const App = () => {
  ...
  const [activeTab, setActiveTab] = useLocalStorage('activeTab', 'clock');
  ...
  useEffect(() => { if (process.env.API_KEY) setHasApiKey(true); }, []);
  ...
  const addTimer = useCallback((timerData, startImmediately = false) => {
  ...
  const deleteTimer = useCallback(id => setTimers(prev => prev.filter(t => t.id !== id)), [setTimers]);
  ...
  const resetTimer = useCallback(id => setTimers(prev => prev.map(t => t.id === id ? {...t, isRunning: false, remaining: t.initialDuration, duration: t.initialDuration, startTime: null} : t )), [setTimers]);
  const updateTimer = useCallback((id, updates) => setTimers(prev => prev.map(t => t.id === id ? { ...t, ...updates } : t)), [setTimers]);
  ...
  return (
      <div className="flex flex-col h-screen max-h-screen bg-background">
          ...
          <SettingsModal isOpen={isSettingsModalOpen} onClose={closeSettingsModal} settings={settings} onSettingsChange={setSettings} />
           <ChatBot isOpen={isChatModalOpen} onClose={closeChatModal} timers={timers} onDeleteTimer={deleteTimer} onToggleTimer={toggleTimer} onResetTimer={resetTimer} onUpdateTimer={updateTimer} alarmSoundUrl={alarmSoundUrl} />
      </div>
  );
};
